...
class DriverController {
   ...
   static async login(req, res) {
      ...
      await DriverModal.find({ phone_number })
         .exec()
         .then((user) => {
            console.log(user);
            if (user.length < 1) {
   ...
   static async findRetrieveDriver(req, res) {
      const user = await DriverModal.findOne({
         phone_number: req.params.phone_number,
      });
      console.log(user);
      if (!user) {
         return Res.handleError(HTTP_NOT_FOUND, 'Usuário não encontrado', res);
      }
      return Res.handleOk(HTTP_OK, user, res);
   }
   /**
    * update user
    * @param {*} req
